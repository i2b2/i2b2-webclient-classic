...
# 2017-07-13 1.0.004 P.K.Ng (URMC) Runs in PHP without errors.
#------------------------------------------------------------------------------------------------------------
  ...
  if (file_exists($tmpZip)) {
    ...
    if (TRUE === $res) {
      ...
      if (file_exists($plugin_folder) && is_dir($plugin_folder)) {
         $t = getdate();
         $old_plugin_folder = $plugin_folder . "_pre" . $t["year"] . $t["mon"] . $t["mday"] . $t["hours"] . $t["minutes"]; //corrected to be real constants...
      ...
      if ($continue) {
       ...
       if ($fc) {
           ...
           if ($file) {
               ...
               print sprintf("<li>%s Please check write access to %s", $red, $retry_loader_phrase);
           }
       } else {
