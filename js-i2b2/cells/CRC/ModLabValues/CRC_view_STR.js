 ...
 * @namespace	i2b2.CRC.view.valueBox.STR
...
i2b2.CRC.view.STR = {
	...
	showDialog: function(panelIndex, queryPanelController, key, extData, isModifier, pluginObj) {
		...
		this.sd = new YAHOO.widget.SimpleDialog("itemLabRange", {
			...
			buttons: [{
				...
				handler: 
					(function() {
						var closure_qpi = i2b2.CRC.view.STR.qpi;
						var closure_number = i2b2.CRC.view.STR.itemNumber;
						// submit value(s)
						if (this.submit()) {
							if(!i2b2.CRC.view.STR.pluginCallerObj){
								...
								for (var i=0; i<pd.items.length; i++) {
									if (pd.items[i].itemNumber==closure_number) {
										if (i2b2.CRC.view.STR.isModifier) {
											pd.items[i].ModValues = i2b2.CRC.view.STR.i2b2Data.ModValues;
										} else {
											pd.items[i].LabValues = i2b2.CRC.view.STR.i2b2Data.LabValues;										
										}
										break;
									}
								}
								...
								i2b2.CRC.view.STR.formatLabValues(i2b2.CRC.view.STR.i2b2Data.itemNumber, pd);
								i2b2.CRC.view.STR.cpc._renameConceptByValueChooser(i2b2.CRC.view.STR.i2b2Data.itemNumber, i2b2.CRC.view.STR.isModifier, pd);
								
								delete i2b2.CRC.view.STR.isModifier;
								i2b2.CRC.view.STR.cpc._redrawDates(pd);
							}
							else
							{
								i2b2.CRC.view.STR.formatLabValues(i2b2.CRC.view.STR.i2b2Data.itemNumber, null);
								i2b2.CRC.view.STR.pluginCallerObj.conceptsRenderFromValueBox();
							}
						}
					})
				}, {
		...
		if (!i2b2.CRC.view.STR.isModifier) {
			if(!i2b2.CRC.view.STR.pluginCallerObj){
				...
				var tmpLab = i2b2.CRC.view.STR.pluginCallerObj.currentTerm.LabValues;
		}
		else
		{
	...
	ValidateSave: function() {
		var dm = i2b2.CRC.view.STR.cfgTestInfo;
		var fd = i2b2.CRC.view.STR.formdata;
		var tmpLabValue = {};
		var errorMsg = [];
		...
		try
		{
			switch (fd.selectedType) {
				case "NONE":
					if (i2b2.CRC.view.STR.isModifier) {
						delete i2b2.CRC.view.STR.i2b2Data.ModValues;
					} else {
						delete i2b2.CRC.view.STR.i2b2Data.LabValues;					
					}
					if(i2b2.CRC.view.STR.pluginCallerObj)
						i2b2.CRC.view.STR.pluginCallerObj.currentTerm.LabValues = i2b2.CRC.view.STR.i2b2Data.LabValues;
					return true;
					break;
				case "VALUE":
			...
			if (i2b2.CRC.view.STR.isModifier) {
				if (tmpLabValue) {
					i2b2.CRC.view.STR.i2b2Data.ModValues = tmpLabValue;
				} else {
					delete i2b2.CRC.view.STR.i2b2Data.ModValues;
				}
			} else { 
				if (tmpLabValue) {
					if(!i2b2.CRC.view.STR.pluginCallerObj)
						i2b2.CRC.view.STR.i2b2Data.LabValues = tmpLabValue;
					else
						i2b2.CRC.view.STR.pluginCallerObj.currentTerm.LabValues = tmpLabValue;
				} else {
					delete i2b2.CRC.view.STR.i2b2Data.LabValues;
				}
			}
	...
	addHandlers: function(){
		...
		YAHOO.util.Event.addListener("mlvfrmStrValue", "keypress", (function(e) {
			...
			var msl = i2b2.CRC.view.STR.cfgTestInfo.valueValidate.maxString;
			if (!msl || this.value.length < msl) {
				delete i2b2.CRC.view.STR.formdata.lastValidStr;
				return true;
			} else {
				if (!i2b2.CRC.view.STR.formdata.lastValidStr) {
					i2b2.CRC.view.STR.formdata.lastValidStr = this.value;
				}
				return true;
			}
		}));
		YAHOO.util.Event.addListener("mlvfrmStrValue", "keyup", (function(e) {
			// anonymous function
			if (i2b2.CRC.view.STR.formdata.lastValidStr) {
				this.value = i2b2.CRC.view.STR.formdata.lastValidStr;
			}
		}));
	},
	...
	formatLabValues: function(key, pd){
		...
		if(!i2b2.CRC.view.STR.pluginCallerObj){
			...
			var values = i2b2.CRC.view.STR.pluginCallerObj.currentTerm.LabValues;
		...
		switch(values.MatchBy) {
			case "VALUE":
				if (values.GeneralValueType=="STRING") {
					...
					formattedLabValues += ' ['+stringOp + i2b2.h.Escape(values.ValueString) + "]";
				}
				break;
			default:
				break;
